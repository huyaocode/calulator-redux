function getanswer(state) {
	let ans;
	try{
        ans = suffixExpression(state.expression)
        ans = ans - 0;
    ...
    let _log = saveLog(state, ans);
	return {expression: ans, equalFlag: true, log:_log};
}
/**
 * 生成记录
 */
function saveLog(state, ans) {
    let { log } = state;
    log.push((state.expression + ' = ' + ans));
    return log;
}
